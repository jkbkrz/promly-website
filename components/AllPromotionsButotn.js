...
export default function AllPromotionsButton({ props }) {
    ...
    return (
        <div className="flex w-full justify-center">
            <div className="bg-lime-500 rounded-full text-center inline-flex px-3 py-2 items-center mt-4 " {...props}>
                <Link className="inline-flex items-center gap-x-1.5 text-sm text-white decoration-2 hover:underline font-medium" href="/promotions">
                    ...
                    {Boolean(count) && <div className='inline-block rounded-full bg-white px-1.5 ml-1 text-xs text-lime-500 dark:text-black'>{count}</div>}
